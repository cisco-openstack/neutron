...
#    under the License.
#
# Original by Edouard Thuleau, Cloudwatt.
# based on
...
_exprs = {'table': re.compile(r'^Bridge table: ([a-z]+)$'),
...
def _process_table(lines):
    ...
    for line in lines:
        if _exprs['comment_or_blank'].search(line):
            continue
        match = _exprs['rule'].search(line)
        if table and match:
            rules += '[%s:%s] -A %s %s\n' % (match.group(1),
                                             match.group(2),
                                             chain,
                                             line[:match.start()].strip())
        match = _exprs['chain'].search(line)
        if match:
            chains += ':%s %s\n' % (match.group(1), match.group(2))
            chain = match.group(1)
            continue
        match = _exprs['table'].search(line)
        if match:
            table = '*%s\n' % match.group(1)
            continue
