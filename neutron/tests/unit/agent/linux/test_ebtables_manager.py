...
_default_indices = {'a1': "0", "a2": "0", "a3": "0", "a4": "0",
                    'a5': "0", "a6": "0", "a7": "0", "a8": "0"}
EBTABLES_ARG = {'pc': ebtables_manager.ChainName.binary_name()}
EBTABLES_ARG.update(_default_indices)
...
_FILTER_DUMP_BASE_2 = \
               ...
               (':%(pc)s-FORWARD ACCEPT\n'
                ':%(pc)s-INPUT ACCEPT\n'
                ':%(pc)s-OUTPUT ACCEPT\n'
                '[%(a1)s:%(a2)s] -A INPUT -j %(pc)s-INPUT\n'
                '[%(a3)s:%(a4)s] -A OUTPUT -j %(pc)s-OUTPUT\n'
                '[%(a5)s:%(a6)s] -A FORWARD -j %(pc)s-FORWARD\n')

_FILTER_DUMP_BASE_3 = \
               _FILTER_DUMP_BASE_2 + '[0:0] -A INPUT -j CONTINUE\n'
...
_FILTER_DUMP_BASE_5 = \
               _FILTER_DUMP_BASE_4 + \
               '[%(a7)s:%(a8)s] -A %(pc)s-filter -p ARP -i eth0 -j DROP\n'

FILTER_DUMP1 = _FILTER_DUMP_BASE + 'COMMIT\n'

FILTER_DUMP2 = _FILTER_DUMP_BASE + \
               (':INPUT ACCEPT\n'
                ':FORWARD ACCEPT\n'
                ':OUTPUT ACCEPT\n'
...
FILTER_DUMP3 = (_FILTER_DUMP_BASE_2 + 'COMMIT\n') % EBTABLES_ARG
FILTER_DUMP4 = (_FILTER_DUMP_BASE_3 + 'COMMIT\n') % EBTABLES_ARG
FILTER_DUMP5 = (_FILTER_DUMP_BASE_4 + 'COMMIT\n') % EBTABLES_ARG
FILTER_DUMP6 = (_FILTER_DUMP_BASE_5 + 'COMMIT\n') % EBTABLES_ARG
...
_NAT_DUMP_BASE_2 = (
            ...
            '[%(a1)s:%(a2)s] -A PREROUTING -j %(pc)s-PREROUTING\n'
            '[%(a3)s:%(a4)s] -A OUTPUT -j %(pc)s-OUTPUT\n'
            '[%(a5)s:%(a6)s] -A POSTROUTING -j %(pc)s-POSTROUTING\n')

_BASE_NAT_DUMP_WITH_NAT = (
               _NAT_DUMP_BASE +
               ':%(pc)s-OUTPUT ACCEPT\n'
               ':%(pc)s-PREROUTING ACCEPT\n'
               ':%(pc)s-nat ACCEPT\n'
...
_BASE_NAT_DUMP_WITH_NAT_ETH0 = (
            _BASE_NAT_DUMP_WITH_NAT +
            '[%(a7)s:%(a8)s] -A %(pc)s-nat -d 00:11:22:33:44:55 -i eth0 '
            '-j dnat --to-destination 54:44:33:22:11:00\n')

NAT_DUMP1 = _NAT_DUMP_BASE + 'COMMIT\n'
NAT_DUMP2 = _NAT_DUMP_BASE + '[0:0] -A PREROUTING -j CONTINUE\nCOMMIT\n'
NAT_DUMP3 = (_NAT_DUMP_BASE_2 + 'COMMIT\n') % EBTABLES_ARG
NAT_DUMP4 = (_BASE_NAT_DUMP_WITH_NAT + 'COMMIT\n') % EBTABLES_ARG
NAT_DUMP5 = _BASE_NAT_DUMP_WITH_NAT_ETH0 + 'COMMIT\n'
...
class EbtablesManagerTestCase(base.BaseTestCase):
    ...
    def test_binary_name(self):
        max_len = ebtables_manager.ChainName.MAX_LEN_PREFIX_CHAIN
    ...
    def test_init_manager_and_rules_add_before_others(self):
        ...
        self.ebtables.apply()
        filter_dump_mod = FILTER_DUMP4
        nat_dump_mod = (_NAT_DUMP_BASE_2 +
                        '[0:0] -A PREROUTING -j CONTINUE\n'
                        'COMMIT\n') % EBTABLES_ARG
    ...
    def test_add_and_remove_chain_with_rule_and_custom_prefix_chain(self):
        ...
        ebtables_args = {'pc': pc}
        ebtables_args.update(_default_indices)
        ...
        self.ebtables.tables['filter'].add_rule('filter',
                                                '-p ARP -i eth0 -j DROP')
        self.ebtables.apply()
        filter_dump_mod = (_FILTER_DUMP_BASE_5 + 'COMMIT\n') % ebtables_args
    ...
    def test_add_and_remove_filter_chain(self):
        ...
        self.ebtables.tables['filter'].add_rule('filter',
                                                '-p ARP -i eth0 -j DROP')
        self.ebtables.apply()
        ebtables_dump_mod = FILTER_DUMP6 + NAT_DUMP3 + BROUTE_DUMP3
        self.ebt_restore.assert_called_with(ebtables_dump_mod,
                                            self.ebtables.ebtables_path,
                                            self.ebtables.execute,
                                            namespace=None)
    ...
    def test_add_and_remove_nat_chain(self):
        ...
        self.ebtables.tables['nat'].add_rule('nat',
                                             ...
                                             '54:44:33:22:11:00')
        self.ebtables.apply()
        nat_dump_mod = NAT_DUMP5 % EBTABLES_ARG
    ...
    def test_add_and_remove_filter_rule(self):
        ...
        self.ebtables.tables['filter'].add_rule('filter',
                                                '-p ARP -i eth1 -j DROP')
        self.ebtables.apply()
        filter_dump_mod = (_FILTER_DUMP_BASE_5 +
    ...
    def test_add_and_remove_nat_rule(self):
        ...
        self.ebtables.tables['nat'].add_rule('nat',
                                             ...
                                             '54:44:33:22:11:00')
        self.ebtables.apply()
        nat_dump_mod = (_BASE_NAT_DUMP_WITH_NAT_ETH0 +
    ...
    def test_empty_filter_chain(self):
        ...
        self.ebtables.apply()
        ebtables_dump_mod = FILTER_DUMP5 + NAT_DUMP3 + BROUTE_DUMP3
        self.ebt_restore.assert_called_with(ebtables_dump_mod,
                                            self.ebtables.ebtables_path,
                                            self.ebtables.execute,
                                            namespace=None)
    ...
    def test_empty_nat_chain(self):
        ...
        self.ebtables.apply()
        ebtables_dump_mod = FILTER_DUMP3 + NAT_DUMP4 + BROUTE_DUMP3
        self.ebt_restore.assert_called_with(ebtables_dump_mod,
                                            self.ebtables.ebtables_path,
                                            self.ebtables.execute,
                                            namespace=None)
    ...
    def test_restore_counters(self):
        ebtables_args = EBTABLES_ARG.copy()
        ebtables_args.update(
                {'a1': "0", "a2": "1", "a3": "2", "a4": "3",
                 'a5': "4", "a6": "5", "a7": "0", "a8": "0"})
        filter_dump = (_FILTER_DUMP_BASE_2 + 'COMMIT\n') % ebtables_args
        broute_dump = ('*broute\n'
                       ':BROUTE ACCEPT\n'
                       ':%(pc)s-BROUTING ACCEPT\n'
                       '[0:1] -A BROUTING -j %(pc)s-BROUTING\n'
                       'COMMIT\n' % EBTABLES_ARG)
        self.ebt_save.return_value = filter_dump + NAT_DUMP3 + broute_dump
        self.ebtables.tables['filter'].add_chain('filter')
        self.ebtables.tables['filter'].add_rule('filter',
                                                '-p ARP -i eth0 -j DROP')
