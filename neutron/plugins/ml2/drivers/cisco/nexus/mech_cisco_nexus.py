...
class CiscoNexusMechanismDriver(api.MechanismDriver):
    ...
    def configure_switch_entries(self, switch_ip, port_bindings):
        ...
        if pvlans:
            ...
            try:
                ...
                self.choose_to_reraise_driver_exception(switch_ip,
                    'replay restore_port_binding')
                LOG.error(_LE("Failed to configure last port binding "
                          "for switch %(switch_ip)s, "
                          "port %(port)s, "
                          "reason %(reason)s"),
                          {'switch_ip': switch_ip,
        ...
        try:
            ...
            self.choose_to_reraise_driver_exception(switch_ip,
                'replay configure_next_batch_of_vlans')
            LOG.error(_LE("Failed to configure first vlan batch"
                      "for switch %(switch_ip)s, "
                      "reason %(reason)s"),
                      {'switch_ip': switch_ip,
                      'reason': e})
    ...
    def _port_action_vlan(self, port, segment, func, vni):
        ...
        is_provider = segment.get(api.PROVIDER_SEGMENT)
    ...
    def update_port_postcommit(self, context):
        ...
        if self._is_vm_migrating(context, vlan_segment, orig_vlan_segment):
            ...
            if (self._is_supported_deviceowner(context.current) and
                ...
                inactive_switches = list(
                    set(all_switches) - set(active_switches))
                for switch_ip in inactive_switches:
                    self.reset_switch_retry_count(switch_ip)
                # if switches not active but host_id is valid
                if not active_switches and all_switches:
                    pass
                else:
                    vni = (self._port_action_vxlan(context.current,
                              vxlan_segment,
                              self._configure_nve_member)
                          if vxlan_segment else 0)
                    self._port_action_vlan(context.current,
                              vlan_segment,
